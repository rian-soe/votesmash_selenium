...
// Coordination variables for multi-user synchronization
let phase1CompletedCount = 0;
let phase2CompletedCount = 0;
let phase3CompletedCount = 0;
let phase4CompletedCount = 0;

// Helper function to wait for all users to complete a phase
async function waitForAllUsersToComplete(phaseName, currentUserIndex, totalUsers) {
    return new Promise((resolve) => {
        const checkCompletion = () => {
            let completedCount;
            switch(phaseName) {
                case 'phase1': completedCount = phase1CompletedCount; break;
                case 'phase2': completedCount = phase2CompletedCount; break;
                case 'phase3': completedCount = phase3CompletedCount; break;
                case 'phase4': completedCount = phase4CompletedCount; break;
                default: completedCount = 0;
            }
            
            if (completedCount >= totalUsers) {
                console.log(`🎯 All ${totalUsers} users completed ${phaseName}`);
                resolve();
            } else {
...
// Helper function to mark phase completion
function markPhaseCompletion(phaseName) {
    switch(phaseName) {
        case 'phase1': phase1CompletedCount++; break;
        case 'phase2': phase2CompletedCount++; break;
        case 'phase3': phase3CompletedCount++; break;
        case 'phase4': phase4CompletedCount++; break;
    }
}
...
async function testTourStepsForUser(userConfig, userIndex) {
    ...
    try {
        ...
        // --- Purchase Tour (if not already purchased) ---
        logStep(`[User ${userIndex + 1}] Checking if tour needs to be purchased`);
        try {
            // First check if "Join Tour" is already available
            const joinTourLink = await driver.findElement(By.linkText("Join Tour"));
            logSuccess(`[User ${userIndex + 1}] Tour already purchased, Join Tour link is available`);
        } catch (error) {
            // If Join Tour is not found, we need to purchase the tour
            logStep(`[User ${userIndex + 1}] Purchasing tour`);
            const purchaseTourButton = await driver.wait(until.elementLocated(By.xpath("//button[contains(text(), 'Purchase Tour')]")), 10000);
            await driver.executeScript("arguments[0].scrollIntoView({ behavior: 'smooth', block: 'center' });", purchaseTourButton);
            await sleep(500);
            await purchaseTourButton.click();

            logStep(`[User ${userIndex + 1}] Confirming purchase`);
            ...
            logStep(`[User ${userIndex + 1}] Submitting purchase`);
            await driver.wait(until.elementLocated(By.css(".px-2")), 10000);
            await driver.findElement(By.css(".px-2")).click();

            logStep(`[User ${userIndex + 1}] Verifying purchase completion`);
            const joinTourSelector = await driver.wait(until.elementLocated(By.linkText("Join Tour")), 10000);
            ...
            if (linkText.includes("Join Tour")) {
                logSuccess(`[User ${userIndex + 1}] Tour purchase completed successfully`);
            } else {
                logFailure(`[User ${userIndex + 1}] Tour purchase failed - Join Tour link not found`);
                throw new Error("Tour purchase failed");
            }
        }
        ...
        // --- Wait for all users to complete Match 1 before time update ---
        markPhaseCompletion('phase1');
        logStep(`[User ${userIndex + 1}] Waiting for all users to complete Match 1...`);
        await waitForAllUsersToComplete('phase1', userIndex, users.length);
        logSuccess(`[User ${userIndex + 1}] All users completed Match 1, proceeding to Match 2`);
        ...
        // --- Wait for all users to complete Match 2 (MVC submission) before time update ---
        markPhaseCompletion('phase2');
        logStep(`[User ${userIndex + 1}] Waiting for all users to complete Match 2 (MVC submission)...`);
        await waitForAllUsersToComplete('phase2', userIndex, users.length);
        logSuccess(`[User ${userIndex + 1}] All users completed Match 2, proceeding to voting phase`);
        ...
        // --- Wait for all users to complete voting before final time update ---
        markPhaseCompletion('phase3');
        logStep(`[User ${userIndex + 1}] Waiting for all users to complete voting...`);
        await waitForAllUsersToComplete('phase3', userIndex, users.length);
        logSuccess(`[User ${userIndex + 1}] All users completed voting, proceeding to tour conclusion`);
        ...
        // --- Mark final phase completion ---
        markPhaseCompletion('phase4');
        logStep(`[User ${userIndex + 1}] Waiting for all users to complete final phase...`);
        await waitForAllUsersToComplete('phase4', userIndex, users.length);
        logSuccess(`[User ${userIndex + 1}] All users completed the tour successfully!`);
